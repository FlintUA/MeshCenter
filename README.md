...
- **On by default for fresh installs - you choose the password in the browser.** The first time MeshCenter starts with no `data/auth.json` yet, nothing is generated: the very first visit to any URL is redirected to a **setup page** (`/setup`, "Welcome to MeshCenter") where you create a password (at least 12 characters, entered twice). Submitting it signs you straight in; from then on `/setup` redirects away and the normal login page (`/login`) applies. You can change the password later via **Settings → Security**. No password is ever generated, logged, or written to disk in plaintext, and there is nothing to look up over SSH.
  - **Existing installs are not affected.** `config.py` is a local, gitignored file - `git pull`/updating never touches it, and every existing install already has `AUTH_ENABLED = False` written into its own `config.py` from whenever it was installed. Nothing changes on an existing install unless you edit `config.py` yourself - and an install that already has `data/auth.json` with a password keeps it.
  - Set `AUTH_ENABLED = False` in `config.py` to opt out of protection (and the setup page) entirely, same as before.
  - **Forgot your password?** SSH in, remove `data/auth.json`, and restart the service (`sudo systemctl restart meshcenter.service`). MeshCenter then shows the `/setup` page again so you can create a new password. This is stricter than the old behaviour: deleting `auth.json` used to switch protection off until you set a new password, which meant the app was served unauthenticated in between. Now it never is - every URL (including `/api/*`, which answers `401` with `error_code: "setup_required"`) is redirected to `/setup` until a password exists. (Requires `AUTH_ENABLED = True` in `config.py`; with `False`, deleting the file simply leaves protection off.)
  - **Regenerating `config.py` from a fresh `config.example.py`** (e.g. comparing against upstream, or a from-scratch reinstall that leaves `data/` behind) will pick up `AUTH_ENABLED = True` again. If `data/auth.json` isn't present at that point, MeshCenter shows the setup page on the next visit, even if you didn't expect a "fresh install" experience.
  - **A fresh instance is claimed by whoever completes `/setup` first.** Do it right after installing, from a device you trust on your own network, rather than leaving a brand-new instance open on a shared or guest network.
- When enabled, an unauthenticated browser is redirected to a login page (`/login`); unauthenticated API requests get `401`. Static assets and the login/setup pages themselves stay reachable so those forms can render.
- The password is hashed (`werkzeug.security`, never stored in plain text) in `data/auth.json`, not in `config.py` or `settings.json` - it isn't wiped by an unrelated settings save and never comes back in a settings API response. `config.py`'s `AUTH_PASSWORD_HASH` is only ever consulted the first time (to seed `auth.json` when set) - once `auth.json` exists, `config.py`'s two auth variables are ignored on every later restart. An `auth.json` that says "enabled" but has no password hash (for example a corrupted or hand-edited file) no longer opens the app up: it forces the `/setup` page instead.
