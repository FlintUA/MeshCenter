...
## Amendment (3rd-pass review): bounded concurrent probing, a structured upload decision, full ACK route validation, TOFU fail-closed on missing delivery, and a pre-merge dependency block

A third independent review found the 2nd-pass amendment's own fixes were each real but incomplete in specific, named ways - detailed in full in `docs/attachments/MCAttach_File_Transfer_Technical.md` (§§4-11) and `docs/attachments/MCAttach_Multi_Relay_Requirements.md` rather than repeated here at length:

1. **Sequential Relay probing was never actually bounded, because it was never concurrent at all.** The 2nd-pass amendment's own note that "the real concurrency ceiling is 1 by construction" was true but not what was being asked for - `refresh()` now probes up to `MAX_CONCURRENT_RELAY_PROBES` (2) due Relays concurrently via a bounded thread pool, with every state mutation and the SQLite write still happening only on the calling (worker) thread afterward, never inside a probe's own thread.

2. **`evaluate_upload_readiness()` does not check `profile.enabled`, and the 2nd-pass amendment's own text incorrectly implied it does.** Corrected there directly. The real, structured answer to "is this Relay upload-ready right now" is the new `ConnectivityMonitor.evaluate_upload_decision()` - checks profile existence, `enabled`, `upload_allowed`, a configured token, the live `RelayState` (a distinct reason each for never-checked/unreachable/identity-mismatched/incompatible), and, when supplied, ciphertext size and requested TTL against the profile's own bounds - returning a named `UploadRejectionReason`, not a bare `bool`. `can_upload_to()` (the 2nd-pass addition) is kept as a thin wrapper over it.

3. **The persisted ACK reply route's `connector_profile_id` was silently ignored even after the 2nd-pass "adapter/connector validation" fix - only `adapter_id` was actually checked.** Both are now required and strictly validated (`MeshtasticTextAdapter` gained a real `connector_profile_id` attribute to validate against). This pass also tightens the *missing*-identity case: the 2nd pass treated a missing `adapter_id` as "trust the currently-configured adapter" (permissive); it now fails closed, the same as a confirmed mismatch, consistent with the fail-closed posture applied everywhere else in this review.

4. **TOFU-to-address verification before encrypting still failed open on a missing or non-`DIRECT` delivery record** - the 2nd-pass fix only checked the transport address when a delivery record happened to already exist and be `DIRECT`. Now requires, in order, a delivery record at all, a `DIRECT` route, and a matching `adapter_id`, before checking any individual recipient's address - any failure excludes every recipient. Rejection log lines were also found to include raw transport addresses, key_ids, and attachment_ids; they now name only which check failed.

5. **Nothing bounded inbound OFFER admission by anything other than transfer_id dedup**, until this pass added the per-source/global ceilings the 2nd-pass amendment's own bullet list already described - confirmed here with a test that reaches the global ceiling by spreading a flood across many distinct sources, not just one.

6. **The dependency-change-on-update fix merged first and detected the change afterward.** It now detects via a plain `git diff` between `HEAD` and the upstream ref *before* any `git merge --ff-only` runs - if dependencies changed, the merge never happens at all, `HEAD` and the working tree stay completely unchanged (confirmed by tests against real git repositories), and `POST /api/updates/apply` returns a blocked, non-2xx response instead of `ok=True`. The frontend (`chat-updates-security.js`) shows a persistent instructions panel and never starts restart polling in that case.

## Amendment (4th-pass review): per-probe HTTP sessions, and a SQLite-safe upload-decision snapshot

A fourth independent review found two further gaps in the 3rd-pass amendment's own fixes - detailed in full in `docs/attachments/MCAttach_File_Transfer_Technical.md` (§6, §10) rather than repeated here at length:

1. **The bounded concurrent Relay probing added in the 3rd pass shared one `requests.Session` across every concurrent probe worker, and the surrounding docstring incorrectly claimed this was safe.** `requests.Session` is not documented as safe for concurrent `.request()` calls from multiple threads - only the underlying `urllib3` connection pool is; the `Session` object itself carries mutable state (cookies, hooks, adapters) a concurrent caller can race on. `ConnectivityMonitor` now accepts an optional `session_factory` (defaulting to `requests.Session` itself), called fresh for every probe; each probe closes its own session deterministically (`_close_session()`) once it returns, success or failure. The pre-existing `session=` constructor parameter is kept for backward compatibility - it wraps the given object in a factory that always returns that same instance, explicit single-shared-session semantics a caller opts into knowingly (safe for a stateless test double, not a substitute for `session_factory` against a real `Session` under real concurrent I/O). The `MAX_CONCURRENT_RELAY_PROBES` (2) ceiling from the 3rd pass is unchanged. Proven by a regression test using production-shaped session objects tracked by instance id, which asserts two concurrently in-flight probes are using genuinely different instances *while one is still blocked* (not merely "they differed after the fact"), and that every session the factory ever created is closed by the time `refresh()` returns.

2. **`evaluate_upload_decision()`/`AttachmentsService.evaluate_upload_readiness()` - the surface a future Step 1.6A REST handler would call directly - read `self._provider_registry.resolve(provider_id)` on whatever thread called it, a live SQLite read from outside the single owning worker thread.** Every other surface in this ADR's amendments has been hardened to the single-owner-SQLite model (2nd-pass amendment above); this one function was missed. `ConnectivityMonitor` now maintains `self._profile_snapshot`, an atomically-published (whole-dict reference swap, atomic under the GIL) in-memory copy of every registered profile - built once eagerly in `__init__` (construction always happens on the startup thread, never a REST thread) and republished on every `refresh()` call (the `AttachmentsService` worker thread, inside its `tick_lock`). `evaluate_upload_decision()` now reads only this snapshot, never `ProviderRegistry`/SQLite directly, so it is correct both before the first `refresh()` (the constructor's own eager build already covers it) and safe to call from any thread thereafter. Proven by a deterministic thread-identity test that wraps `ConnectivityMonitor`'s own `provider_registry` reference in a call-tracking proxy, invokes `evaluate_upload_decision()` from a separate simulated "REST" thread, and asserts zero registry calls were recorded from that thread - reverting the fix makes this test reproduce a genuine `sqlite3.ProgrammingError` ("SQLite objects created in a thread can only be used in that same thread"), confirming this was a live bug, not a style preference.
