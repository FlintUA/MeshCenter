...
### 9. Transaction hygiene: `_atomic` is top-level-only, and every worker catch rolls back

`service._atomic` is a **top-level service transaction**, not a nestable savepoint: it requires a clean connection on entry and raises `RuntimeError` if `conn.in_transaction` is already true (an outer transaction the caller owns). The earlier implementation released only its own savepoint but then unconditionally `commit()`ed, which inside a caller-owned outer transaction silently committed *the caller's* uncommitted work — a correctness bug, so failing fast is the only safe behavior. Every caller (the reject/revoke command handlers, the per-row expiry sweep) opens `_atomic` from a clean connection, so the guard is never expected to fire in normal operation.

The worker also rolls back at **every exception boundary that catches and continues**, so the shared SQLite connection is always left with no uncommitted changes from a failed unit of work: the inbound-event drain (`_drain_inbound_events`), the automatic row-scan (`_tick_locked`'s `_due_rows` loop), and the per-row expiry sweep (`_reconcile_receiver_expiry`) each call `_rollback_silently()` before logging-and-continuing. This closes the gap where a `run_step`/`_process_one_inbound_event` that wrote before raising would otherwise have its partial write silently committed by a later, unrelated commit in the same tick.
...
- **`service.py`**: `_command_reject()` and `_command_revoke()` transition + enqueue in one transaction; `_command_revoke()` gates CANCEL on SENT/RECEIVED; new `_cancel_route_for()` and `_reconcile_receiver_expiry()` (wired into `_tick_locked()` after the row-scan, before dispatch); `_dispatch_outgoing_replies()` gains the six-step route validation (Decision 6). `_atomic()` gains a clean-connection entry guard (Decision 9), and the inbound drain, the `_due_rows` row-scan loop, and the expiry sweep each call `_rollback_silently()` at their catch-and-continue boundary (Decision 9).
