...
The two facts stay in **two layers, never merged into the published map** (final correction, Finding 2): `_published` holds only the **persisted base** — states derived from persisted worker-owned facts (`waiting_response` / `retry_available` / absence-for-`idle`); `refresh()` reads the persisted timestamps+bindings and publishes that base alone, never consulting the pending-marker map. The **live `queued` overlay** is the lock-guarded, command-id-keyed pending map (`mark_queued()`/`mark_drained()`); `snapshot()` copies it under the lock and overlays `queued` on top of the base at read time, so a marker removed between a worker's read and its publish (a `CommandQueueFull` rollback) can never resurrect a stale `queued` in the base, and a removed marker disappears from the very next `snapshot()` with no worker tick.
...
This ADR ships the shared model and store, plus the **minimal functional wiring** of the store's selection into the Files workspace and the chat sidebar (review Finding 1): every node/channel/MCA-contact click goes through the one store (`store.toggleSelect()`); Files subscribes to the store and mirrors a node selection into its counterparty filter; a second click on the selected target deselects it and returns the full transfer list; a channel selection is retained in the store but is never a file counterparty; and the Send dialog pre-selects a recipient only when the selection is a valid, still-sendable node — an invalid/disappeared/channel selection yields an explicit empty placeholder, never a silent fallback to another recipient. Selection state and `aria-pressed` stay synchronized across views because both read the same store.

The store's selection is the **single source of truth**, and chat.js keeps two facts separate (final correction, Finding 1): *whether the store exists* versus *what its current selection is*. When the store is present, a `null` selection is authoritative — nothing is selected anywhere (no node card, no channel/DM chat item, no Files counterparty), even though the open conversation (`currentChatId`) may still be open; opening a chat and selecting a target are distinct facts. The open-conversation fallback applies **only** when the store is absent (older page / `targets.js` not loaded). The store subscription synchronously re-syncs both the node-card highlight *and* the channel/DM chat-item highlight (matched by an explicit `data-target-kind`, never inferred from a display name) without a network reload. The node/channel/DM target elements are keyboard-accessible (`role="button"` + `tabindex="0"` + Enter/Space activation, with Space-scroll and auto-repeat double-fire prevented) but there is no accessibility refactor outside those target elements.
