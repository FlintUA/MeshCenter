...
- MeshCenter caps how many incoming file offers it will keep track of at once, both from any one sender and overall - so a flood of bogus "here's a file" messages from a misbehaving or malicious node cannot fill up your device's storage with tracking records.
- Checking for new messages never gets stuck waiting on a slow or unreachable Relay - that check runs entirely separately from the part of MeshCenter that watches the radio, so a struggling file transfer never makes your regular chat messages feel slow or unresponsive.
