...
**Contextual upload readiness (`can_upload_to()`, 2nd-pass addition).** `RelayStatus.state` and `.upload_readiness` are reported side by side, deliberately independent (§10 below) - but neither alone answers "is it actually sensible to start an upload to this Relay right now", and `ProviderRegistry.get_upload_candidates()` filters purely on local config with no live-state awareness. `ConnectivityMonitor.can_upload_to(provider_id)` is the missing combined predicate: `True` only when both `can_attempt_relay()` (live reachability) and `evaluate_upload_readiness() == READY` (local config) agree.
...
`evaluate_upload_readiness(profile)` (a public function; previously a private `_upload_readiness_for()`) computes `ready | upload_token_missing | upload_disabled` from a profile's local configuration alone (`upload_allowed`, `upload_token_configured`) - deliberately independent of live `RelayState`, since a Relay can be `upload_readiness=ready` while currently `UNREACHABLE`, or vice versa; the two are reported side by side, never merged. `LIMIT_EXCEEDED` was removed - nothing in this codebase computes a per-Relay upload quota. See §6 above for `ConnectivityMonitor.can_upload_to()`, the combined local-config-and-live-reachability predicate added to answer "is it actually sensible to upload here right now", which neither this function nor `RelayStatus.state` alone can.
...
`meshsrv/update_service.py`'s `apply_update()` detects (never installs - see its own comment on why running pip from inside the live Flask process would be its own hazard) whether `requirements.txt`/`adapters/meshtastic/requirements.txt` changed as part of an applied update. `POST /api/updates/apply` (2nd-pass correction) now skips its automatic post-update restart when that is true, rather than restarting into code that expects a dependency nothing has installed yet - which would very likely crash-loop the service under systemd. The operator is told (console log line and the response's own `requirements_changed`/`restarted` fields) to install dependencies and restart manually.
