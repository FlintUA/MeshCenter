...
- **Independent connectivity tracking:** `ConnectivityMonitor` tracks each registered profile's `RelayState` independently - `unknown|online|degraded|unreachable|identity_mismatch|incompatible|disabled` - keyed by `provider_id`. One Relay being unreachable never marks another Relay (or the workspace's own internet status) as down; `refresh()`'s internet-status derivation only falls back to the generic connectivity probe when *every* registered, non-disabled Relay is failing, or when none are registered yet at all (see ADR-0008's PR #231 amendment for the exact condition and the reviewer-found bug this fixes).
