...
- **Registration:** a workspace can register any number of distinct Relay profiles. Nothing in `ProviderRegistry.register()` limits the count.
...
- **Contextual upload readiness:** `ConnectivityMonitor.can_upload_to(provider_id)` combines live reachability (`can_attempt_relay()`) with local upload configuration (`evaluate_upload_readiness() == READY`) into the one predicate that actually answers "should I start an upload to this Relay right now" - `ProviderRegistry.get_upload_candidates()` alone only checks local config and has no live-state awareness.
...
- **A REST API surface for any of this.** Step 1.6A (the HTTP layer that would expose Provider Registry management, connectivity snapshots, and contact status to a browser) is explicitly out of scope for the PR #231 review and not implemented by it.
