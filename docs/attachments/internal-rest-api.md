...
**Status:** Design contract (Step 1.6A), revised (second pass). The read-only endpoints of sub-stage 1.6A.2 (§5) are implemented in `api/api_attachments.py`. Of the mutation endpoints, only the three Step 1.6A.3A lifecycle actions — `POST /api/attachments/{id}/retry`, `/download`, and `/reject` (§7.3) — are implemented (worker handlers in `meshsrv/attachments/service.py`); every other mutation endpoint (multipart create, cancel, request-key, save, revoke, local-content, and the remaining sub-stages 1.6A.3–1.6A.5) remains design-only and does not exist yet.
