...
- **Responses:** `202 {"ok": true, "command_id", "attachment_id"}`; pending idempotent replay `202 {"ok": true, "command_id", "attachment_id", "replayed": true}` (same ids); committed idempotent replay `200 {"ok": true, "attachment_id", "state"}`; `409 idempotency_conflict`; `400 mime_not_allowed` / `file_too_large` / `provider_not_found` / `ttl_out_of_range` / `invalid_metadata`; `429 command_queue_full`. `recipient_not_found` / `recipient_not_trusted` are **asynchronous** command-result error codes (§7.9), observed by polling `GET /api/mca/commands/{command_id}` — never synchronous `400`s.
