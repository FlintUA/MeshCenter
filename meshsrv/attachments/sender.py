...
# PR 2.5 (outbound CANCEL): the `mca_outgoing_replies.event_type` used when a
# sender-side CANCEL control message is enqueued (the outbox dedup key and the
# `attachment_events.event_type` recorded once dispatch confirms the send).
# Lives here (not in receiver.py's ACK/REJECTED/EXPIRED constants) because
# CANCEL is the one sender -> receiver control message - generated by
# `AttachmentsService._command_revoke()`, not by receiver.py.
CANCEL_EVENT_TYPE = "cancel_sent"
...
def _set_state(
    ...
    *,
    ...
    # PR 2.5: `commit=False` defers the commit to the caller, so a state
    # transition and an outbox insertion (e.g. revoke -> CANCEL) can share one
    # transaction instead of two sequential commits. Default True keeps every
    # existing single-transition caller/tests byte-for-byte unchanged.
    if commit:
        conn.commit()
...
def revoke(
    ...
    PR 2.5: `commit=False` defers the commit to the caller so the REVOKED
    transition and the outbound CANCEL enqueue (service.py) share one
    transaction. Default True keeps the existing local-only callers/tests
    unchanged."""
    now = _now() if now is None else now
    row = _row(conn, attachment_id)
    ...
    _set_state(conn, attachment_id, REVOKED, now, commit=commit)
    conn.execute("DELETE FROM mca_sender_state WHERE attachment_id = ?", (attachment_id,))
